...
const Complaint = () => {
  ...
  const validatePhoneNumber = (phone) => {
    if (!phone) {
      setPhoneError(
        t("complaint.modal.phoneRequired", {
    ...
    if (!cleanNumber.startsWith("9")) {
      setPhoneError(
        t("complaint.modal.mustStart9", {
          defaultValue: "Mobile Numbers must start with 9",
        })
      );
      setIsValidPhone(false);
      return false;
    ...
    if (cleanNumber.length !== 9) {
      setPhoneError(
        t("complaint.modal.invalidLength", {
          defaultValue:
            "Mobile Number must be 9 digits after country code",
        })
      );
      setIsValidPhone(false);
      return false;
  ...
  const handleGetOtp = async () => {
    ...
    try {
      ...
      if (response?.success) {
        ...
        toast.error(
          response?.message ||
            t("complaint.modal.otpFailed", { defaultValue: "Failed to send OTP" })
        );
        setOtpRequested(false);
      }
    } catch (error) {
  ...
  const handleVerifyOtp = async () => {
    if (verificationCode.length !== 6) {
      toast.error(
        t("complaint.modal.invalidOtp", {
          defaultValue: "Please enter a valid 6-digit OTP code",
        })
      );
      return;
    }
    ...
    if (!otpId) {
      toast.error(
        t("complaint.modal.otpIdMissing", {
          defaultValue: "OTP ID not found. Please request OTP again.",
        })
      );
      return;
    }
    ...
    try {
      ...
      const response = await APICall(
        "post",
        data,
        EndPoints.customer.guestVerifyOtp
      );
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6">
            <div className="mb-4">
              ...
              <p className="text-sm mt-2 text-gray-600">
                {t("complaint.modal.description", {
                  defaultValue: "Enter your mobile number to view blocked numbers",
                })}
              </p>
            </div>
            ...
            <div className="border-t border-gray-200 py-4">
              <div className="mb-4">
                <label className="block text-sm text-gray-600 mb-1">
                  {t("complaint.modal.mobileNumber", {
                ...
                <div className="relative flex items-center w-full">
                  ...
                  {verifyingPhone ? (
                    ...
                    text-xs font-medium rounded-lg`}
                      ...
                      >
                        {otpRequested
                          ? t("complaint.modal.resendOtp", {
                              defaultValue: "Resend OTP",
                            })
                          : t("complaint.modal.getOtp", {
              ...
              {expirationTime && !otpVerified && (
                <div className="mb-4">
                  <label className="block text-sm text-gray-600 mb-1">
                    {t("complaint.modal.verificationCode", {
                  ...
                  <div className="relative flex items-center w-full">
                    <input
                      type="text"
                      placeholder={t("complaint.modal.otpPlaceholder", {
                    ...
                    {!otpVerified && (
                      <button
                        ...
                        className={`absolute right-3 bg-gray-100 p-2 shadow-sm border border-gray-200 
                    ${
                      verificationCode.length !== 6 || verifyingOtp
                        ? "opacity-50 cursor-not-allowed"
                        : "cursor-pointer hover:bg-gray-200"
                    } 
                    text-xs font-medium rounded-lg`}
                      ...
                      >
                        {verifyingOtp
                          ? t("complaint.modal.verifying", {
                              defaultValue: "Verifying...",
                            })
                          : t("complaint.modal.verifyOtp", {
