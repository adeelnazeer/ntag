...
function CloseAccount() {
    ...
    const processCloseAccount = () => {
        ...
        APICall("post", payload, EndPoints.customer.updateStatus)
            .then(res => {
                if (res?.success) {
                    toast.success(res?.message || t("closeAccount.toastMessages.successfullyClosedAccount"));
                    handleCloseModal()
                    // Refresh tag data after successful unsubscribe
                } else {
                    toast.error(res?.message || t("closeAccount.toastMessages.failedToCloseAccount"));

                }
                setUnsubscribing(false);
            })
            .catch(err => {
                toast.error(err?.message || t("closeAccount.toastMessages.anErrorOccurred"));
                setUnsubscribing(false);
            });
    };
    ...
    const renderTagItem = (tag) => {
        return (
            <tr key={tag?.id}>
                <td className="py-4 px-4 text-sm text-gray-700">{userData?.comp_name || t("closeAccount.common.na")}</td>
                <td className="py-4 px-4 text-sm text-gray-700">{formatPhoneNumberCustom(userData?.phone_number || t("closeAccount.common.na"))}</td>
                <td className="py-4 px-4 text-sm text-gray-700">
                    {moment(userData?.created_date).format("YYYY-MM-DD") || t("closeAccount.common.na")}
                </td>
                <td className="py-4 px-4 text-sm text-gray-700">
                ...
                <td className="py-4 px-4 text-sm text-gray-700">
                    ...
                    >
                        {unsubscribing && selectedTagId === tag?.reserve_tag_id ? t("closeAccount.buttons.processing") : t("closeAccount.buttons.closeAccount")}
                    </Button>
                </td>
            </tr>
        );
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full bg-white">
                                <thead className="bg-[#F6F7FB]">
                                    <tr>
                                        <th className="py-3 px-4 text-left text-xs font-medium text-[#7A798A]">{t("closeAccount.table.companyName")}</th>
                                        <th className="py-3 px-4 text-left text-xs font-medium text-[#7A798A]">{t("closeAccount.table.mobileNumber")}</th>
                                        <th className="py-3 px-4 text-left text-xs font-medium text-[#7A798A]">{t("closeAccount.table.createdDate")}</th>
                                        <th className="py-3 px-4 text-left text-xs font-medium text-[#7A798A]">{t("closeAccount.table.status")}</th>
                                        <th className="py-3 px-4 text-left text-xs font-medium text-[#7A798A]">{t("closeAccount.table.action")}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {renderTagItem()}
