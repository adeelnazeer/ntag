  ...
  const processResubscribe = (corp_subscriber_id) => {
    ...
    APICall("post", payload, "customer/re-subscribe")
      .then((res) => {
        if (res?.success) {
          toast.success(
            res?.message || t("toastMessages.successfullyResubscribed")
          );
    ...
    APICall("post", payload, "/customer/cancel-reservation")
      .then((res) => {
        if (res?.success) {
          ...
          toast.error(
            res?.message || t("toastMessages.failedToCancelReservation")
          );
        }
        setCancelling(false);
      })
  ...
  const handleConfirmStatusChange = async (item) => {
    ...
    try {
      ...
      const response = await APICall(
        "post",
        payload,
        EndPoints.customer.createMandateCorp
      );
  ...
  const renderTagData = (single) => {
    ...
    return (
      <div
        ...
        key={single?.id}
      >
        {single?.is_recurring_acceptance == 0 &&
          showMessage == false &&
          single?.status != 6 &&
          single?.payment_method == "telebirr" && (
            <Typography className="text-[14px] font-bold mb-3">
              ...
              <span className=" md:block hidden text-blue-600">
                {t("actionRequired.message")} {t("actionRequired.pinPrompt")}{" "}
        ...
        {!isReserved && isPaid && (
          <>
            {single?.fee_charge_date && (
              <div className="flex justify-between md:px-5 px-2 py-3 rounded-xl mt-1">
                <Typography className="text-[14px]">
                  {t("tagInfo.recurringFeeLastChargeDate")}
                </Typography>
                <Typography className="md:text-[14px] text-[12px]">
        ...
        <div className="flex justify-center  mx-auto gap-4 mt-4">
          ...
          {!isReserved && isPaid && isUnsub && (
            ...
            >
              {resubscribing
                ? t("buttons.processing")
                : t("buttons.resubscribeTagNumber")}
            </Button>
          )}
        </div>
      </div>
          ...
          <>
            ...
            <div className="grid md:grid-cols-2 grid-cols-1 gap-4">
              {Array.isArray(tagData) &&
                tagData.map((single) => renderTagData(single))}
            </div>
