    ...
    APICall("post", payload, "individual/unsubscribe")
      .then((res) => {
        if (res?.success) {
          toast.success(
            res?.message ||
              t("profile.tagDetail.toastMessages.successfullyUnsubscribed")
          );
          ...
          toast.error(
            res?.message ||
              t("profile.tagDetail.toastMessages.failedToUnsubscribe")
          );
        }
      })
      .catch((err) => {
        toast.error(
          err?.message || t("profile.tagDetail.toastMessages.anErrorOccurred")
        );
      });
  };
    ...
    APICall("post", payload, "customer/re-subscribe")
      .then((res) => {
        if (res?.success) {
          toast.success(
            res?.message ||
              t("profile.tagDetail.toastMessages.successfullyResubscribed")
          );
          ...
          toast.error(
            res?.message ||
              t("profile.tagDetail.toastMessages.failedToResubscribe")
          );
        }
      })
      .catch((err) => {
    ...
    APICall("post", {}, `/individual/cancel-reservation/${reserve_tag_id}`)
      .then((res) => {
        if (res?.success) {
          toast.success(
            res?.message ||
              t("profile.tagDetail.toastMessages.reservationCanceled")
          );
          ...
          toast.error(
            res?.message ||
              t("profile.tagDetail.toastMessages.failedToCancelReservation")
          );
        }
      })
      .catch((err) => {
      ...
      >
        ...
        <div className="flex justify-between bg-[#F6F7FB] md:px-3 px-2 py-3 rounded-xl items-center">
          <div className="flex items-center gap-3">
            <Typography className="md:text-[14px] text-[12px]">
              {isNotPremium
                ? single?.tag_name
                : single?.premium_tag_list?.tag_name ||
                  t("profile.tagDetail.na")}
            </Typography>
          </div>
          <div>
            <Typography className="text-[14px] bg-secondary py-1 px-4 rounded-lg text-white">
              ...
              {isNotPremium
                ? single?.tag_no
                : single?.premium_tag_list?.tag_no || t("profile.tagDetail.na")}
            </Typography>
          </div>
        </div>
        ...
        <div className="flex justify-between border border-blue-200 bg-blue-50 md:px-5 px-2 py-3 rounded-xl mt-3">
          ...
          <Typography className="text-[14px] font-bold text-blue-600">
            {isNotPremium
              ? single?.tag_type
              : single?.premium_tag_list?.tag_type || t("profile.tagDetail.na")}
          </Typography>
        </div>
        ...
        <div className="md:px-5 px-2 py-3 rounded-xl mt-1">
          <div className="flex justify-between">
            <Typography className="text-[14px]">
              {isReserved
                ? t("profile.tagDetail.reserveDate")
                : t("profile.tagDetail.subDate")}{" "}
            </Typography>
            <Typography className="md:text-[14px] text-[12px]">
        ...
        <div className="flex justify-between md:px-5 px-2 py-3 rounded-xl mt-1">
          <Typography className="text-[14px]">
            {t("profile.tagDetail.subFee")}
          </Typography>
          <Typography className="md:text-[14px] text-[12px]">
            {formatPrice(
              isNotPremium
                ? single?.tagnoprice
                : single?.premium_tag_list?.tag_price || "0"
            )}{" "}
        ...
        <div className="md:px-5 px-2 py-3 rounded-xl mt-1">
          <div className="flex justify-between">
            <Typography className="text-[14px]">
              {t("profile.tagDetail.serviceStatus")}{" "}
            </Typography>
            <Typography className="md:text-[14px] text-[12px] capitalize">
              {single?.type == "reserve"
                ? t("profile.tagDetail.reserved")
        ...
        {!isReserved && isPaid && (
          <div className="flex justify-between md:px-5 px-2 py-3 rounded-xl mt-1">
            <Typography className="text-[14px]">
              {t("profile.tagDetail.incomingCallsStatus")}
            </Typography>
            <Typography className={`md:text-[14px] text-[12px] font-medium `}>
              {single?.service_status == 1
                ? t("profile.tagDetail.on")
                : t("profile.tagDetail.off")}
            </Typography>
          </div>
        )}
        ...
        <div className="flex justify-between md:px-5 px-2 py-3 rounded-xl mt-1">
          <Typography className="text-[14px]">
            ...
            {t("profile.tagDetail.recurringFee")}
          </Typography>
          <Typography className="md:text-[14px] text-[12px]">
            {formatPrice(
              single?.service_id === t("profile.tagDetail.monthly")
        ...
        {!isReserved && isPaid && (
          <>
            {single?.fee_charge_date && (
              <div className="flex justify-between md:px-5 px-2 py-3 rounded-xl mt-1">
                <Typography className="text-[14px]">
                  {t("profile.tagDetail.serviceFeeLastChargeDate")}
                </Typography>
                <Typography className="md:text-[14px] text-[12px]">
                  {single?.fee_charge_date ||
        ...
        {!isReserved && isPaid && !isUnsub ? (
          <div className="flex justify-between md:px-5 px-2 py-3 rounded-xl mt-1">
            <Typography className="text-[14px]">
              {t("profile.tagDetail.recurringFeeNextDueDate")}
            </Typography>
            <Typography className="md:text-[14px] text-[12px]">
      ...
      {loading ? (
        ...
        <div className="md:p-8 p-4">
          ...
          <div className="gridgrid-cols-1 gap-4">
            {Array.isArray(state) &&
              state?.map((single) => renderTagData(single))}
          </div>
