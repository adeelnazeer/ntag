...
//       toast.error(
//         `Please enter a ${
//           blockType === "mobileNumber" ? "mobile number" : "TAG number"
//         } to block`
//       );
//       return;
//     }
...
//           `${
//             blockType === "mobileNumber" ? "Mobile number" : "TAG number"
//           } added to blocklist successfully`
//         );
...
const BlockUnblock = () => {
  ...
  const [addingNumber, setAddingNumber] = useState(false);
  const [deletingNumber, setDeletingNumber] = useState(false);
  const [blockType, setBlockType] = useState("mobileNumber");
  const [valueToBlock, setValueToBlock] = useState("+2519");
  const [mobileNumbers, setMobileNumbers] = useState([]);
  const [selectedMsisdn, setSelectedMsisdn] = useState("");
  ...
  const fetchMobileNumbers = async () => {
    ...
    try {
      ...
      console.error("Error fetching Mobile numbers:", error);
      toast.error(t("errors.loadMobiles"));
      setMobileNumbers([]);
    } finally {
      setLoading(false);
    }
  ...
  const fetchBlockedNumbers = async (msisdn) => {
    try {
      ...
      console.error("Error fetching blocked numbers:", error);
      toast.error(t("errors.loadBlocked"));
      setNumbers([]);
    } finally {
      setLoading(false);
    }
  ...
  const confirmDelete = async () => {
    try {
      ...
      if (deleteResponse?.success) {
        ...
        toast.error(t("errors.removeBlocked"));
      }
    } catch (error) {
      console.error("Error deleting blocked number:", error);
      toast.error(t("errors.removeBlocked"));
  ...
  const addNewNumber = async () => {
    if (!valueToBlock) {
      toast.error(
        blockType === "mobileNumber"
          ? t("errors.requiredToBlockMobile")
          : t("errors.requiredToBlockTag")
      );
      return;
    }

    if (blockType === "mobileNumber") {
      if (!validatePhoneNumber(valueToBlock)) return;
    }
        ...
        <div className="fixed p-2 inset-0 z-[999] grid h-screen w-screen place-items-center bg-black bg-opacity-60 backdrop-blur-sm">
          <div className="relative w-full max-w-md rounded-2xl bg-white p-6">
            ...
            <div className="mt-4 text-center">
              ...
              <Typography className="mt-2 text-sm text-gray-600">
                {t("confirmDialog.desc1")}
              </Typography>
              <Typography className="mt-2 text-sm text-gray-600">
                {t("confirmDialog.desc2")}
              </Typography>
            </div>
