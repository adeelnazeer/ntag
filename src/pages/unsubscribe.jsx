...
function Unsubscribe() {
    ...
    APICall("post", payload, "customer/unsubscribe")
      .then((res) => {
        if (res?.success) {
          toast.success(
            res?.message ||
              t("unsubscribe.toastMessages.successfullyUnsubscribed")
          );
          // Refresh tag data after successful unsubscribe
          fetchTagData();
        } else {
          toast.error(
            res?.message || t("unsubscribe.toastMessages.failedToUnsubscribe")
          );
        }
      ...
      .catch((err) => {
        toast.error(
          err?.message || t("unsubscribe.toastMessages.anErrorOccurred")
        );
        setUnsubscribing(false);
      });
  };
  ...
  const renderTagItem = (tag) => {
    ...
    const tagInfo =
      isPremium && tag?.corp_premium_tag_list
        ? tag?.corp_premium_tag_list
        : tag?.corp_tag_list || {};

    return (
      <tr key={tag?.id}>
        <td className="py-4 px-4 text-sm text-gray-700">
          #{tagInfo?.tag_no || t("unsubscribe.common.na")}
        </td>
        <td className="py-4 px-4 text-sm text-gray-700">
          {formatPhoneNumberCustom(tag?.msisdn || t("unsubscribe.common.na"))}
        </td>
        <td className="py-4 px-4 text-sm text-gray-700">
          {tag?.created_date ? tag.created_date : t("unsubscribe.common.na")}
        </td>
        <td className="py-4 px-4 text-sm text-gray-700">
        ...
        <td className="py-4 px-4 text-sm text-gray-700">
          ...
          >
            {unsubscribing && selectedTagId === tag?.reserve_tag_id
              ? t("unsubscribe.buttons.processing")
              : t("unsubscribe.buttons.unsubscribe")}
          </Button>
        </td>
      </tr>
    );
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full bg-white">
                  <thead className="bg-[#F6F7FB]">
                    <tr>
                      <th className="py-3 px-4 text-left text-xs text-[#7A798A]">
                        {t("unsubscribe.table.tagNumber")}
                      </th>
                      <th className="py-3 px-4 text-left text-xs text-[#7A798A]">
                        {t("unsubscribe.table.mobileNumber")}
                      </th>
                      <th className="py-3 px-4 text-left text-xs text-[#7A798A]">
                        {t("unsubscribe.table.registrationDate")}
                      </th>
                      <th className="py-3 px-4 text-left text-xs text-[#7A798A]">
                        {t("unsubscribe.table.status")}
                      </th>
                      <th className="py-3 px-4 text-left text-xs text-[#7A798A]">
                        {t("unsubscribe.table.action")}
                      </th>
                    </tr>
                  </thead>
                  <tbody>{tagData.map((tag) => renderTagItem(tag))}</tbody>
                </table>
              </div>
            )}
          </>
